...
export default function Memory() {
  ...
  const handleAdd = async () => {
    if (!formKey.trim() || !formContent.trim()) {
      setFormError(t('memory.validation_required'));
      return;
    }
    setSubmitting(true);
    setFormError(null);
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-white">{t('memory.add_title')}</h3>
              <button
                onClick={() => {
                  setShowForm(false);
                  setFormError(null);
                  ...
                  <td className="px-4 py-3 text-right">
                    {confirmDelete === entry.key ? (
                      <div className="flex items-center justify-end gap-2">
                        <span className="text-xs text-red-400">{t('common.delete_question')}</span>
                        <button
