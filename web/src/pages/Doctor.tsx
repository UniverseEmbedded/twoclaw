...
export default function Doctor() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('doctor.failed_run_fallback'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-white">{t('doctor.title_short')}</h2>
        </div>
        <button
          onClick={handleRun}
          disabled={loading}
        ...
        <>
          ...
          <div className="flex items-center gap-4 bg-gray-900 rounded-xl border border-gray-800 p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white font-medium">
                {warnCount}{' '}
                <span className="text-gray-400 font-normal">{t('doctor.warn')}</span>
              </span>
            </div>
            <div className="w-px h-5 bg-gray-700" />
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white font-medium">
                {errorCount}{' '}
                <span className="text-gray-400 font-normal">{t('doctor.error')}</span>
              </span>
            </div>
      ...
      {!results && !loading && !error && (
        <div className="flex flex-col items-center justify-center py-16 text-gray-500">
          ...
          <p className="text-sm mt-1">{t('doctor.empty')}</p>
        </div>
      )}
    </div>
  );
